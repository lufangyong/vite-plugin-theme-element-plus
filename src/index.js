...
export default function themePreprocessorPlugin(options = {}) {
  ...
  return {
    name: "vite-plugin-theme-element-plus/theme",
    enforce: "pre",
    api: {
      getOptions() {
    ...
    config(conf, { command }) {
      ...
      optimizeDeps.exclude = [
        "vite-plugin-theme-element-plus/theme/dist/browser-utils",
        "vite-plugin-theme-element-plus/theme/dist/browser-utils.js",
      ].concat(
...
function themePreprocessorHmrPlugin() {
  ...
  return {
    // 插件顺序必须post
    enforce: "post",
    name: "vite-plugin-theme-element-plus/theme-hmr",
    config(conf, { command }) {
      buildCommand = command
    },
    configResolved(resolvedConfig) {
    ...
    buildStart() {
      // 获取依赖插件提供的 方法
      const parentName = "vite-plugin-theme-element-plus/theme"
